...
public class SalesAnalyzer {
    public static void main(String[] args) {
        ...
        Map<String, Double> byCategory = sales.stream().collect(groupingBy(Sale::category, summingDouble(Sale::amount)));
        ...
        Map<String, Double> avgByCategory = sales.stream().collect(groupingBy(Sale::category, averagingDouble(Sale::amount)));
        System.out.println("\n=== Promedio por Categoria ===");
        avgByCategory.forEach((cat, avg) -> System.out.printf("  %s: $%,.2f%n", cat, avg));

        // 6. Producto mas caro por region
        System.out.println("\n=== Producto mas Caro por Region ===");
        // TODO: groupingBy region, maxBy comparando amount
        Map<String, Optional<Sale>> bestPerRegion = sales.stream().collect(groupingBy(Sale::region, maxBy(Comparator.comparingDouble(Sale::amount))));
        bestPerRegion.forEach((region, sale) -> sale.ifPresent(s -> System.out.printf("  %s: %s ($%,.2f)%n", region, s.product(), s.amount())));

        // 7. Conteo por mes
        Map<Integer, Long> byMonth = sales.stream().collect(groupingBy(s -> s.date().getMonthValue(), counting()));
        System.out.println("\n=== Ventas por Mes ===");
        byMonth.forEach((month, count) -> System.out.printf("  Mes %d: %d ventas%n", month, count));
    }
}